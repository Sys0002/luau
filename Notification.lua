local notiftheme = {
	MainColour = Color3.fromRGB(13, 20, 25),
	BackgroundColour = Color3.fromRGB(13, 20, 25),
	SecondaryBackgroundColour = Color3.fromRGB(13, 20, 25),
	Success = Color3.fromRGB(13, 20, 25),
	Error = Color3.fromRGB(13, 20, 25),
	Alert = Color3.fromRGB(13, 20, 25),
...
function notiftheme.failSafeChecks(custom_sex)
	-- // If custom_sex is not provided or custom_sex is not a table
	if not custom_sex or (custom_sex and type(custom_sex) ~= "table") then custom_sex = notiftheme
		return custom_sex
	end
	 
	-- // If custom_sex's keys are incorrect or nil
	for i,v in pairs(notiftheme) do
		for a,x in pairs(custom_sex) do
			if (not custom_sex[i]) or (typeof(custom_sex[i]) ~= typeof(v)) then
				custom_sex[i] = v
			end
		end
	end
...
function notiftheme.returnBaseNotification(custom_sex)
	custom_sex = notiftheme.failSafeChecks(custom_sex)
	...
	if not custom_sex["StorageLocation"]:FindFirstChild(custom_sex["GUIName"]) then
		local NotificationGUI = Instance.new("ScreenGui")
		NotificationGUI.Name = custom_sex["GUIName"]
		NotificationGUI.Parent = custom_sex["StorageLocation"]
		NotificationGUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	end
	local NotificationGUI = custom_sex["StorageLocation"]:WaitForChild(custom_sex["GUIName"])
	Notification.Name = custom_sex["NotificationName"]
...
function notiftheme.getY(custom_sex)
	local Highest = 0.80
	local FrameCount = 0
	local NotificationGUI = custom_sex["StorageLocation"]:WaitForChild(custom_sex["GUIName"])
...
function notiftheme.newNotification(TitleText, BodyText, NotificationType, custom_sex)
	-- // Initialise
	coroutine.wrap(function()
		TitleText = tostring(TitleText)
		custom_sex = notiftheme.failSafeChecks(custom_sex)
		local Notification = notiftheme.returnBaseNotification(custom_sex)	
		local NotificationGUI = custom_sex["StorageLocation"]:WaitForChild(custom_sex["GUIName"])
		local Theme = notiftheme
		...
		if not NotificationType or (typeof(NotificationType) ~= 'string' and typeof(NotificationType) ~= 'Color3') or (typeof(NotificationType) == 'string' and not custom_sex[NotificationType]) then NotificationType = "Success" end

		if type(NotificationType) == 'string' then
			NotificationType = custom_sex[NotificationType]
		end
		 
		-- // Actually making the Notification
		Notification.Position = UDim2.new(0, 0, notiftheme.getY(custom_sex), 0)
		...
		Notification.BorderColor3 = custom_sex["MainColour"]
		Notification.BackgroundColor3 = custom_sex["BackgroundColour"]
		Notification.StatusBar.BackgroundColor3 = custom_sex["MainColour"]
		...
		Notification.Title.Font = custom_sex["TitleFont"]
		
		Notification.Body.BackgroundColor3 = custom_sex["SecondaryBackgroundColour"]
		Notification.Body.Text = BodyText
		Notification.Body.Font = custom_sex["BodyFont"]
		...
		if Notification and Notification:FindFirstChild("StatusColour") and Notification:FindFirstChild("StatusBar") then
			...
			coroutine.wrap(function()
				Notification.StatusBar:TweenSize(UDim2.new(0, 400, 0, 100), "In", "Quad", 0.22)
				wait(0.22)
